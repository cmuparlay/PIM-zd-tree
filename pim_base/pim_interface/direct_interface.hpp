...
class DirectPIMInterface : public PIMInterface {
   ...
   public:
    DirectPIMInterface(dpu_set_t dpu_set) : PIMInterface(dpu_set) {
        this->initiate_finished = false;
        init_all_buffer_pointers();
        load_from_dpu_set(this->dpu_set);
        this->initiate_finished = true;
    }
   ...
   private:
    bool initiate_finished;

    void free_all_buffer_pointers() {
        if (ranks != nullptr) {
            delete[] ranks;
        }
